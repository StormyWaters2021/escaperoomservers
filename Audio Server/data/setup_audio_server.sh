...
# Repo root is provided by the master installer (falls back to default)
...
DISABLE_FLAG_BOOT="/boot/firmware/server.disable"
...
ensure_venv(){
  ...
  "$VENV_DIR/bin/pip" install --upgrade numpy pyalsaaudio || true
}
...
ConditionPathExists=!${DISABLE_FLAG_BOOT}
...
do_stop(){ require_root; systemctl stop "$SERVICE"; }
do_restart(){ require_root; systemctl restart "$SERVICE"; }
...
do_info(){
  # Print human-readable info for master installer
...
Commands:
  ...
  info           Print service/app/port info (no changes)
EOF
}
